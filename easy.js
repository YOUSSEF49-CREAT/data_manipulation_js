...
//     let x = 0
//     for(let i = 0 ; i < employees.length ; i++){
//         x += (employees[i].age)/employees.length
//     }
//     return x ;
// }
...
let emplo = document.getElementById('employees-tbody') 
...
button.addEventListener('click',()=>{
    if(varx === 0){
       ...
       <tr>
                ...
                <th>${employees[i].Développement}</th>
                <th>${employees[i].position}</th>
                <th>${employees[i].salary}</th>
                <th>${employees[i].email}</th>
       </tr>
       
       `
      div.innerHTML = table
      emplo.appendChild(div)
      }
    }else{
